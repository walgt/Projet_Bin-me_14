...
public class ConstumTestRig {
    ...
    public static void process(TestConfig config, ArrayList<TESTBaseListener> listeners, ANTLRErrorListener errorListener) throws Exception {


        TESTLexer lexer = new TESTLexer(null);
        CommonTokenStream tokens = new CommonTokenStream(lexer);
        TESTParser parser = new TESTParser(tokens);
        parser.removeErrorListener(ConsoleErrorListener.INSTANCE);
        parser.addErrorListener(errorListener);

        for (TESTBaseListener listener : listeners) parser.addParseListener(listener);
    ...
    private static void process(TESTLexer lexer, TESTParser parser, CharStream input, TestConfig config) throws IOException, IllegalAccessException, InvocationTargetException, PrintException {
